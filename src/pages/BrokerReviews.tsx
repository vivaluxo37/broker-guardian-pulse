...
const assetTypes = ["Stock, ETF", "Forex", "Fund", "Bond", "Options", "Futures", "Crypto", "CFD"];
const BrokerReviews = () => {
  const {
    t
  } = useTranslation();
  ...
  const filteredBrokers = brokers.filter(broker => {
    ...
    return matchesSearch && matchesAssets && matchesFeeLevel && matchesRating && matchesInvestorProtection && matchesMobilePlatform && matchesAwardWinner;
  });
  const renderStars = (rating: number) => {
    return <div className="flex items-center gap-1">
        {[...Array(5)].map((_, i) => <Star key={i} size={16} className={i < Math.floor(rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"} />)}
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      ...
      <div className="container mx-auto px-4 py-8 mt-20 max-w-7xl">
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-6 shadow-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="Type broker name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="border-slate-200 focus:border-primary transition-colors duration-200" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                {assetTypes.map(asset => <Badge key={asset} variant={selectedAssets.includes(asset) ? "default" : "outline"} className={`cursor-pointer px-4 py-2 text-sm font-medium transition-all duration-200 ${selectedAssets.includes(asset) ? "bg-primary text-white hover:bg-primary/90 shadow-md" : "border-slate-300 text-slate-600 hover:border-primary hover:text-primary hover:bg-primary/5"}`} onClick={() => toggleAssetFilter(asset)}>
                    {asset}
                  </Badge>)}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {filteredBrokers.map(broker => <Card key={broker.id} className="group relative hover:shadow-2xl transition-all duration-300 border border-slate-200 bg-white shadow-sm overflow-hidden">
              {broker.recommended && <div className="absolute -top-2 -right-2 bg-gradient-to-r from-accent to-accent/80 text-slate-800 px-5 py-2.5 rounded-full text-sm font-bold shadow-lg z-10 whitespace-nowrap">
                  ⭐ Recommended
                </div>}
              ...
              <CardHeader className="space-y-6 p-8">
                <div className="flex items-start justify-between">
                  <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm">
                    <img src={broker.logo} alt={broker.name} className="w-14 h-14 object-contain filter contrast-110 brightness-110" style={{
                  maxWidth: '100%',
                  maxHeight: '100%'
                }} />
                  </div>
                  {broker.isAwardWinner && <div className="flex items-center gap-2 bg-gradient-to-r from-amber-400 to-amber-500 text-amber-900 px-3 py-2 rounded-full shadow-md">
                      <Trophy size={16} />
              ...
              <CardContent className="space-y-6 p-8 pt-0">
                ...
                <div className="flex justify-between items-center py-3 border-b border-slate-100">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${broker.feeLevel === 'Low' ? 'bg-green-100 text-green-700' : broker.feeLevel === 'Average' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                      {broker.feeLevel}
                    </span>
                ...
                <div className="flex justify-between items-center py-3 border-b border-slate-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {broker.mobileRating && <div className="flex items-center">
                ...
                {/* Risk Disclaimer */}
                {broker.riskDisclaimer && <div className="text-xs text-slate-600 bg-amber-50 border border-amber-200 p-4 rounded-lg">
                ...
                <div className="flex gap-3 pt-4">
                  <Button className="flex-1 border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-all duration-200" variant="outline">
                    Read Review
                  </Button>
        ...
        {/* No Results Message */}
        {filteredBrokers.length === 0 && <div className="text-center py-16">
